...
<section class="p-4 sm:p-6 bg-white rounded shadow-md overflow-x-auto">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <h1 class="text-lg sm:text-xl font-semibold mb-4 md:mb-0">Programs</h1>
        <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0">
            ...
            <div>
                <form method="GET" action="" class="flex">
                    ...
                    <input type="text" id="searchInput" name="search" placeholder="Search programs..."
            ...
            <!-- Removed rows per page dropdown beside search bar as requested -->
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <?php if (!empty($programs)): ?>
                    <?php foreach ($programs as $index => $program): ?>
                        <tr>
                            ...
                            <td class="px-4 py-2 whitespace-nowrap"><?php echo htmlspecialchars($program['ProgramName']); ?>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-center space-x-2">
                                <button class="text-blue-600 hover:text-blue-900 edit-btn" title="Edit"
                                    ...
                                    data-program-name="<?php echo htmlspecialchars($program['ProgramName']); ?>"
                                    aria-label="Edit">
                                ...
                                <button class="text-red-600 hover:text-red-900 delete-btn" title="Delete"
                                    data-program-id="<?php echo $program['ProgramID']; ?>" aria-label="Delete">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="3" class="px-4 py-2 text-center text-gray-500">No programs found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<div id="addNewProgramModal"
    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
        ...
        <form id="addProgramForm" method="POST" action="src/partials/dashboard_pages/admin/functions/func_programs.php">
            <div class="mb-4">
                <label for="addProgramName" class="block text-gray-700 font-medium mb-2">Program Name</label>
                <input type="text" id="addProgramName" name="addProgramName" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
            <div class="flex justify-end space-x-2">
                <button type="button" data-modal-hide="addNewProgramModal"
                    class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400 focus:outline-none">Cancel</button>
                <button type="submit" name="addProgram"
...
<div id="editProgramModal"
    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
        ...
        <form id="editProgramForm" method="POST"
            action="src/partials/dashboard_pages/admin/functions/func_programs.php">
            <input type="hidden" id="editProgramID" name="editProgramID">
            <div class="mb-4">
                <label for="editProgramName" class="block text-gray-700 font-medium mb-2">Program Name</label>
                <input type="text" id="editProgramName" name="editProgramName" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
            <div class="flex justify-end space-x-2">
                <button type="button" data-modal-hide="editProgramModal"
                    class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400 focus:outline-none">Cancel</button>
                <button type="submit" name="editProgram"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const addModal = document.getElementById('addNewProgramModal');
        ...
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                if (confirm('Are you sure you want to delete this program?')) {
                    // Send AJAX request to delete the program
                    fetch('src/partials/dashboard_pages/admin/functions/func_programs.php', {
                        method: 'POST',
                        headers: {
                        ...
                        .catch(error => {
                            messageContainer.innerHTML = '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">Error deleting program.</div>';
    ...
    // AJAX form submission for Edit Program
